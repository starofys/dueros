 ...
 */
public class AccessTokenManager implements Serializable {
    // accessToken信息
...
//    }
    ...
    protected void initToken() {
        this.accessToken = OauthPreferenceUtil.getAccessToken();
        long expires = OauthPreferenceUtil.getExpires();
        long createTime = OauthPreferenceUtil.getCreateTime();
    ...
    protected void clearToken() {
        OauthPreferenceUtil.clearAllOauth();
     ...
     */
    protected void storeToken(Map<String,Object> values) {
        if (values == null || values.isEmpty()) {
            return;
        }
        this.accessToken = (String)values.get("access_token");
        // expires_in 返回值为秒
        long expiresIn = Long.parseLong((String)values.get("expires_in")) * 1000;
        this.expireTime = System.currentTimeMillis() + expiresIn;
        OauthPreferenceUtil.setAccessToken( this.accessToken);
        OauthPreferenceUtil.setCreateTime(System.currentTimeMillis());
        OauthPreferenceUtil.setExpires(expiresIn);
    }
